...
function ContactSection({
  ...
  const quickWhatsappMessage =
    whatsappMessage ||
    `Hi ${brand.name}, I want to book a trial for the ${selectedPlan} plan.`;
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          <div>
            ...
            <p className="mt-4 max-w-lg text-sm leading-6 text-muted-foreground sm:text-base">
              Share your goal and preferred plan. The team will confirm your
              first session slot.
            </p>
          ...
          <Card className="contact-card">
            <CardHeader>
              <CardHeading>
                <CardTitle className="pt-2.5">Book a trial</CardTitle>
                <CardDescription>
                  Selected membership: {selectedPlan}
                </CardDescription>
              </CardHeading>
            </CardHeader>
            <CardContent>
              <form className="contact-form grid gap-4" onSubmit={handleSubmit}>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div>
                    <label className="sr-only" htmlFor="trial-goal">
                      Primary goal
                    </label>
                    <Input
                      id="trial-goal"
                ...
                <label className="sr-only" htmlFor="trial-notes">
                  Preferred time or training history
                </label>
                <textarea
                  id="trial-notes"
                  name="notes"
                ...
                {submitted && (
                  <div className="contact-success">
                    <div role="status">
                      Thanks - your {selectedPlan} trial details are ready in
                      WhatsApp. Tap send to share it with the gym.
                    </div>
                    <div
                      className="contact-success-actions"
                      aria-label="Send trial details on WhatsApp"
                    >
                      {brand.whatsappNumbers.map((whatsappNumber) => (
                        <Button
                          key={whatsappNumber.number}
                          asChild
                          size="sm"
                          variant="outline"
                        >
  ...
  return (
    <div className="contact-item flex items-center gap-3">
      ...
      <div className="min-w-0">
        ...
        <div className="contact-item-value text-sm text-muted-foreground">
          {value}
        </div>
      </div>
    </div>
